...
items = pd.read_csv('./data/bronze/items.csv', index_col=0)
items.head()
...
actual_ratings = pd.read_csv('./data/bronze/ratings.csv', index_col=0)
actual_ratings.head()
...
content_based = pd.read_csv('./data/gold/cbf.csv', index_col=0)
user_user = pd.read_csv('./data/gold/user-user.csv', index_col=0)
item_item = pd.read_csv('./data/gold/item-item.csv', index_col=0)
matrix_fact = pd.read_csv('./data/gold/mf.csv', index_col=0)
pers_bias = pd.read_csv('./data/gold/pers-bias.csv', index_col=0)


# In[39]:


comma_float_cols = ['Availability','Price']
items[comma_float_cols] = items[comma_float_cols].apply(lambda col:                                                         col.apply(lambda elem: str(elem).replace(',', '.'))).astype(float)
...
# 
# In order to make it easier to evaluate the metrics, let's create a class that receives all the original ratings and predicted ratings for every recommender system and defined functions to extract all the metrics. Lets take a look at a summary of the class before looking at the code:
...
# ## Hibridization Techniques
# 
# We are trying four different types of hybridization here.
# 
# 1. Linear ensemble
